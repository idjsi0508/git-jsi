...
let isRaining = true;
let now = isRaining == true ? '비가 오고 있습니다.' : '비가 오지 않습니다.';
console.log(`현재 ${city}의 온도는 ${temperature}도이며, ${now}`);
// console.log(`현재 ${city}의 온도는 ${temperature}도이며, ${isRaining ? '비가 오고 있습니다' : '비가 오지 않습니다'}`); 18번 줄을 주석하고 이 코드 사용 가능
...
let magic; // 연산의 결과
// num에 number prompt 부여 magic에 계산을 다 해도 됨
// ex) magic = num * 2 +8 ..

// 입력한 숫자에 아래의 연산을 작성
num1 = num = prompt('숫자를 입력해 주세요');
// 숫자에 2를 곱하기
num *= 2;
magic = num
// 결과에 8을 더하기
...
//  5000점 이상이면 트로피나 우승을 안 해도 되는 지 물어보기
...
console.log(typeof hasTrophy);
let gameWins = Number(prompt('플레이어가 우승한 게임의 수를 입력하세요.'));

let isEligible = score >= 5000 && (hasTrophy === ture || gameWins >= 3);  // 위의 모든 조건을 만족할 경우 해당 변수에 true값 저장
// ! todo 점수만 true이고 나머지는 false일 때 오류가 뜸
// isEligible의 값을 출력
