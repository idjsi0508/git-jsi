...
// 2) 결합 방식: 수식 내에 우선순위가 같으면 같은 연산자가 둘 이상 있을 때, 먼저 어느 연산을 수행할 것인가를 결정
...
let n = 2;
let k = l = m = n;
console.log(`k의 값은 ${k}입니다.`); // k의 값은 2
//& ---------------------------
...
x = 10;
console.log(x++); // 후위 증가 연산자: +1이 해당 값에서 바로 적용 X - 10이 출력될 것
console.log(x++); // 11
console.log(x); // 12
...
x = 10;
console.log(--x); // 전위 감소 연산자: -1이 해당 값에서 바로 적용 - 9가 출력될 것
console.log(--x); // 8
...
// 빼기 후 할당
a -= 2; // a = a - 2
console.log(a); // 15 - 2 = 13

// 곱셈 후 할당
a *= 2; // a = a * 2
console.log(a); // 13 * 2 = 26

// 나누기 후 할당
a /= 2; // a = a / 2
console.log(a); // 26 / 2 = 13
