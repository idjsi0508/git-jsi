...
// >> 변수 삽입
let name = '조신익';
let greeting = `안녕하세요 ${name}님`; // 그리팅이라는 변수에 할당
...
let multiLine1 = '안녕하세요'; // 일반 따옴표의 경우 여러줄 문자 작성이 불가
...
bool = true;
console.log(bool); // true로 표현

//? null & undefined 자료형

// null
// : 변수에 값이 '없을'을 명시적으로 나타냄
// : 변수가 '비어 있음'을 의도적으로 표현하고자 할 때 사용
// (변수에 할당된) 데이터의 값이 유효하지 않은 상태

console.log(null);
console.log(typeof null); // object - 비워져있지만 해당 데이터는 객체라는 구조를 따름

// undefined
// : 변수가 선언되었으나, 아직 어떠한 값도 할당되지 않은 변수의 기본값
let hello;
...
console.log(numberToBoolean); // ture
