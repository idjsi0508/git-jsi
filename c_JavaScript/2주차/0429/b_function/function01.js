...
//? 3. 이벤트 처리
// : 웹 페이지에서 발생하는 다양한 이벤트(버튼 클릭, 폼 제출 등)를 처리하는데 에 사용
...
// const 변수명 = function(매개변수) {
  ...
  // const 변수명 = (매개변수) => {
    ...
    // const hi = (name) => {
      ...
      // 위와 동일한 형식 매게변수가 하나면 중괄호와 소괄호 생략가능
      ...
      // 함수 선언 - 함수만의 스코프{} 영억이 생성
      function lsa() {
        // 지역 변수(함수 스코프 내에서만 적용)
        let job = ' Developer';
        console.log(job);
        return sinik + job; // 문자열 연산자(+)
      }

      console.log(lsa()); // sinIk Developer
      // console.log(job); - Error (지역 변수의 값에 접근하지 못함)

      //! 함수 선언 방식에 대한 베스트 프렉티스
      // - 일반적 상황에서는 함수 선언문 방식을 사용 - 기본
      // - 조건부 함수 선언 시 함수 표현식 방식을 사용 - ex) let num1 = function() {}
