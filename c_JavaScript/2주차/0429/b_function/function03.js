...
function defaultParams(parma1='defaultValue1', parma2=2) {}
...
greet(); // 안녕하세요 이승아님
// : 전달되는 인자가 없을 경우 - 파라미터에 초기화되지 않는 경우 기본 매게변수가 사용
...
function exampleFunction(param1, param2, ...args) { // args >> arguments의 축약어
  // let param1;
  // let param2;
  // let args = [];
...
function findMin(...numbers) {
  // numbers 배열을 Math.min()에 전달
  // : 최소값을 반환
  return Math.min(...numbers);
}

let minResult = findMin(58, 35, 92, 10, 23, 77); // -(마이너스)도 가능
...
function combinedExample(firstParam, ...restOfParams) {
  ...
  console.log(`첫 번째 매게 변수는 ${firstParam}, 타입: ${typeof firstParam}`);

  // 배열의 형태 확인
  // : JS의 내장 함수인 Array.isArray(배열명)으로 확인 가능 - Boolean 타입으로 반환
  console.log(`나머지 매게변수의 형태는 배열입니까? ${Array.isArray(restOfParams)}`);
