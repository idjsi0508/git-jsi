...
sports.length = 4;
console.log(sports); // <2 empty items >> undefined 의 값이 2개
...
console.log(index); // 첫 번째 초코송이가 1번 인덱스에 있으므로 1 반환
...
console.log(lastIndexOfSnacks); // 마지막 초코송이가 3번 인덱스에 있으므로 3 반환

//? 배열명.includes()
// : 배열에 특정 요소가 존재하는지 확인
// : 그 결과를 '불리언 값'으로 반환 (true/false) 
//   (요소의 인덱스X, 존재 유무만 확인)
...
console.log(newLength); // 2개의 인덱스에 하나를 추가했기 때문에 3 반환

// 요소 삭제
// pop(): 배열의 마지막 요소를 제거, 그 요소를 반환
// - 빈 배열의 경우 undefined를 반환
let lastFruit = fruits.pop();
console.log(lastFruit); // 마지막 인덱스가 cherry 이기 때문에 제거하고 반환
console.log(fruits);

// shift(): 배열의 첫 번째 요소를 제거, 그 요소를 반환
// - 빈 배열의 경우 undefined를 반환
let firstFruit = fruits.shift();
console.log(firstFruit); // 첫 번째 인덱스가 apple 이기 때문에 제거하고 반환

// unshift(): 배열의 시작 부분에 하나 이상의 요소를 추가
// - 수정된 배열의 새로운 길이를 반환
newLength = fruits.unshift('strawberry');
console.log(newLength); // 하나의 인덱스에 하나를 추가했기 때문에 2 반환

// splice
// *splice(a, b, c) - 삭제, 추가, 교체 가능
// a - 삭제할 요소의 시작 인덱스
// b - 삭제할 요소의 수
// c - 추가할 아이템 나열

// : 배열의 기존 요소를 삭제하거나 새로운 요소를 추가하거나의 모든 동작을 수행
console.log(fruits); // 인덱스 수 확인 >> 'strawberry', 'banana'

// - 요소 삭제: 인자값이 2개
// : splice(삭제할 요소의 시작 인덱스, 삭제할 요소의 수)
let removedItems = fruits.splice(1, 1);
console.log(removedItems); // 1번 인덱스부터 1개 삭제 >> banana 삭제 및 출력
...
console.log(fruits); // 1번 인덱스부터 2개 삭제 후 'banana', 'Apple' 나열

//! 배열 조작 메서드
//? 배열 정렬 & 순서 변경
// sort(): 배열의 요소를 정렬 - 문자열 순서대로 정렬(오름차순)
fruits = ['Apple', 'strawberry', 'cherry', 'watermelon', 'melon']
console.log(fruits); // [ 'strawberry', 'banana', 'Apple' ]
let sortArray = fruits.sort();
console.log(sortArray); // 대문자 A부터 ~ Z까지

let mixNumbers = [52, 32, 15, 74, 43]
console.log(mixNumbers.sort()); // 숫자가 작은 순서 대로

// reverse(): 배열의 요소의 순서를 반대로 정렬(내림차순)
let reverseArray = fruits.reverse();
console.log(reverseArray); // 대문자 Z부터 ~ A까지
console.log(mixNumbers.reverse()); // 숫자가 큰 순서대로