...
//? reduce(): 배열의 각 요소에 대해 함수를 적용하여 '단일 값을 생성' >> sum등 로직에 주로 사용

//& 기본 형태(구조)
// 배열명.reduce((accumulator(축적), currentValue(값), currentIndex(인덱스), array) => {
  // 함수 로직
// }, initialValue);
...
// : 특정 요소의 인덱스를 찾는 메서드
// : 중복된 값이 있을 경우 첫 번째 요소의 인덱스번호를 반환
// : 중복된 값이 있을 경우 마지막 요소의 인덱스번호를 반환
...
// findInedx(): 해당 조건을 만족하는 첫 번째 요소의 인덱스번호를 반환
let firstOverFiveIndex = numbers.findIndex(num => num > 5);

console.log(firstOverFive); // 6
console.log(firstOverFiveIndex); // 3번째 인덱스에 6이 있으므로 3 반환 